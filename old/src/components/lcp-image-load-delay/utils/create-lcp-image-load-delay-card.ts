import type { LcpImageLoadDelayMetricName } from "../types";
...
/**
 * Создает DOM-элемент для метрики LCP Image Load Delay.
 *
 * @param metricName - Название метрики
 * @param value - Значение метрики (p75)
 * @returns DOM-элемент карточки метрики
 */
export function createLcpImageLoadDelayCard(
  metricName: LcpImageLoadDelayMetricName,
  value: number | null,
): HTMLElement {
  ...
  description.textContent =
    "Задержка между обнаружением LCP-изображения и началом его загрузки. Высокое значение может указывать на проблемы с приоритетами загрузки или lazy loading.";
