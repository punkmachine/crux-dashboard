import type { LcpImageRenderDelayMetricName } from "../types";
...
/**
 * Создает DOM-элемент для метрики LCP Image Render Delay.
 *
 * @param metricName - Название метрики
 * @param value - Значение метрики (p75)
 * @returns DOM-элемент карточки метрики
 */
export function createLcpImageRenderDelayCard(
  metricName: LcpImageRenderDelayMetricName,
  value: number | null,
): HTMLElement {
  ...
  description.textContent =
    "Задержка между завершением загрузки LCP-изображения и его отрисовкой на экране. Высокое значение может указывать на проблемы с декодированием изображения или блокировкой основного потока.";
